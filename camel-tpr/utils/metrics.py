...
class Evaluator():
    ...
    def _compute_embedding_local(self, model:DATPS):
        ...
        for pid, caption in self.txt_loader:
            caption = caption.to(device)
            with torch.no_grad():
                text_feat = model.encode_text_local(caption)
            qids.append(pid.view(-1)) # flatten 
            qfeats.append(text_feat)
        qids = torch.cat(qids, 0)
        qfeats = torch.cat(qfeats, 0)
        ...
        for pid, img in self.img_loader:
            img = img.to(device)
            with torch.no_grad():
                img_feat = model.encode_image_local(img)
            gids.append(pid.view(-1)) # flatten 
            gfeats.append(img_feat)
        gids = torch.cat(gids, 0)
        gfeats = torch.cat(gfeats, 0) 
    ...
    def eval(self, models, i2t_metric=False, return_table=False, print_log=True):
        ...
        sims_dict = {
            ...
            'LE': sims_local  / len(models),
            'GE+LE': (sims_global+sims_local)/ (2 * len(models))
        }
